...
//            PP_RELEASE(alterView);
        ...
        {
            _hostName = [httpServer_ hostName];
            if(_hostName==nil||[_hostName isEqualToString:@"null"])
            {
                if(completion)
                {
                    NSError * error = [NSError errorWithDomain:@"cn.net.veritrans.DocolFileServer" code:-1005 userInfo:@{NSLocalizedDescriptionKey:@"请打开WiFI，再开启服务。"}];
                    completion(error);
                }
                NSLog(@"%@", @"请打开WiFI，再开启服务");
            ...
            {
                NSLog(@"vdcserver:    http://%@:%d", _hostName,[httpServer_ port]);
                if(completion)
                {
                    completion(nil);
                }
