...
- (NSString *) getFileNameByTicks:(NSString *)fileName
{
    ...
    if(r.location !=NSNotFound)
    {
        return [NSString stringWithFormat:@"%@_%ld%@",
                ...
                [NSDate date].miniSeconds,
                [fileName substringFromIndex:r.location]
                ];
    }
    else
    {
        return [NSString stringWithFormat:@"%@_%ld",fileName,[NSDate date].miniSeconds];
    }
}
...
+ (NSString *)getMD5FileNameKeepExt:(NSString *)orgPath defaultExt:(NSString *)defaultExt
{
    ...
    return [NSString stringWithFormat:@"%@.%@",[orgPath MD5],ext];
}
